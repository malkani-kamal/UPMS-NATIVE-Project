# This will create the transport channel
#  No arguments needed if created the default transport channel in the project
function usage {
    echo "create-channel.sh   [ORDERER_ADDRESS default=7050] [CHANNEL_NAME default=transportchannel] [CHANNEL_TX_FILE default=../../artefacts/airline-channel.tx]"
    echo "                    Make sure to execute . set-env.sh before calling this scripy"
}

# Defaults for the channel name and tx file path
CHANNEL_NAME=transportchannel
CHANNEL_TX_FILE=../../artefacts/transport-channel.tx 
